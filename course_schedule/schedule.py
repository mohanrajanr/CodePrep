...
first_node = [node for node in range(numCourses) if len(dependents[node]) == 0]
...
def is_safe(index, res):
    main_set = dependents[index]
    if len(main_set) == 0:
        return True

    print("Safe Set:{} RES:{} index:{}".format(main_set, res, index))
    return len(main_set.intersection(res)) == len(res)
...
def dfs(res):
    ...
    for index in range(numCourses):
        ...
        if is_safe(index, set(res)):
            print("Safe:{}".format(index))

            visited.add(index)

            res_list = dfs(res + [index])
            if res_list:
                return res_list
